...
nat_init (vlib_main_t * vm)
{
  ...
  sm->ip4_lookup_main = &sm->ip4_main->lookup_main;
...
nat_config (vlib_main_t * vm, unformat_input_t * input)
{
      ...
      else
	{
	  ...
	  if (unformat (input, "udp timeout %u", &udp_timeout));
	    ...
	    ;
		   ...
		   (input, "outside ip6 VRF id %d", &outside_ip6_vrf_id))
	    ;
	  else
  ...
  // TODO: create timeout data structure so it can be passed efficiently
  ...
  switch (nat_type)
    ...
    case NAT_TYPE_64:
      ...
      // TODO: refactor and change to:
      //        tcp_transitory_.../tcp_established_...
      nm64->tcp_trans_timeout = tcp_transitory_timeout;
      nm64->tcp_est_timeout = tcp_established_timeout;
      nm64->outside_vrf_id = outside_ip6_vrf_id;
  ...
  nm44->max_translations = 10 * nm44->translation_buckets;
  vec_add1 (nm44->max_translations_per_fib, nm44->max_translations);
  nm44->max_translations_per_user =
    nm44->max_translations_per_user ==
    ~0 ? nm44->max_translations : nm44->max_translations_per_user;
  ...
  nm44->outside_vrf_id = outside_vrf_id;
  nm44->inside_vrf_id = inside_vrf_id;
  nat_log_err ("running nat44");
