...
vl_api_nat_det_forward_t_handler (vl_api_nat_det_forward_t * mp)
{
  ...
  out_addr.as_u32 = 0;
  clib_memcpy (&in_addr, mp->in_addr, 4);
  dm = snat_det_map_by_user (&in_addr);
  if (!dm)
    {
...
vl_api_nat_det_reverse_t_handler (vl_api_nat_det_reverse_t * mp)
{
  ...
  in_addr.as_u32 = 0;
  clib_memcpy (&out_addr, mp->out_addr, 4);
  dm = snat_det_map_by_out (&out_addr);
  if (!dm)
    {
      rv = VNET_API_ERROR_NO_SUCH_ENTRY;
      goto send_reply;
...
vl_api_nat_det_session_dump_t_handler (vl_api_nat_det_session_dump_t * mp)
{
  ...
  dm = snat_det_map_by_user (&user_addr);
  if (!dm)
    return;
...
static void
vl_api_det44_interface_dump_t_handler (vl_api_nat44_interface_dump_t * mp)
{
  det44_main_t *dm = &det44_main_t;
  vl_api_registration_t *reg;
  det44_interface_t *i;
...
static void
vl_api_det44_set_timeouts_t_handler (vl_api_nat_set_timeouts_t * mp)
{
...
static void
vl_api_det44_get_timeouts_t_handler (vl_api_nat_get_timeouts_t * mp)
{
